...
// For type T we expect it to have fields next and prev, which are T pointers, because that way we can optimize our space complexity.
template<class T>
class List 
{
protected:
...
T* List<T>::take_first() {
    ...
    if (!this->is_empty()) {
        ...
        if (!this->head) {
            // If head pointed to the last element, now its nullptr, then let tail also be nullptr.
            this->tail = nullptr;
        }
        else {
...
T* List<T>::take_last() {
    ...
    if (!this->is_empty()) {
        ...
        if (!this->tail) {
            // If the tail pointed to the last element, now it is nullptr, so set the head to be nullptr as well.
            this->head = nullptr;
        }
        else {
            // In case it wasn't the last element, we know that the current tail shouldn't have the next element.
